...
public class CaseService {
    ...
    private final Map<UUID, DispatchAssignedEvent> pendingDispatches = new ConcurrentHashMap<>();
    private final Map<UUID, HospitalAssignedEvent> pendingHospitals = new ConcurrentHashMap<>();
    ...
    private synchronized void tryCreateCase(UUID emergencyId) {
        if (pendingDispatches.containsKey(emergencyId) && pendingHospitals.containsKey(emergencyId)) {
            ...
            DispatchAssignedEvent dispatch = pendingDispatches.get(emergencyId);
            HospitalAssignedEvent hospital = pendingHospitals.get(emergencyId);

            Instant now = Instant.now();
            EmergencyCase emergencyCase = new EmergencyCase(
                    emergencyId, dispatch.ambulanceId(), hospital.hospitalId(), now, now
            );
            ...
            CaseCreatedEvent caseCreated = new CaseCreatedEvent(
                    UUID.randomUUID(),
                    Instant.now(),
                    savedCase.getCaseId(), emergencyId, dispatch.ambulanceId(), hospital.hospitalId(), hospital.hospitalName()
            );
